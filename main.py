...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    Use `matplotlib` to generate a figure of the distance & #expanded-nodes
     w.r.t. the weight.
    DONE [Ex.12]: Complete the implementation of this method.
    """
    ...
    p1, = ax1.plot(weights, total_cost, label='Solution cost', color='b')  # DONE: pass the relevant params instead of `...`.
    ...
    p2, = ax2.plot(weights, total_nr_expanded, label='#Expanded states', color='r')  # DONE: pass the relevant params instead of `...`.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    weights_array = np.linspace(low_heuristic_weight, high_heuristic_weight, num=n)
    costs_list = []
    num_expanded_list = []
    weights_list = []
    for weight in weights_array:
        ...
        if(res.is_solution_found):
            costs_list.append(res.solution_g_cost)
            num_expanded_list.append(res.nr_expanded_states)
            weights_list.append(weight)

    assert len(costs_list) == len(num_expanded_list) == len(weights_list)

    plot_distance_and_expanded_wrt_weight_figure(problem.name, weights_list, costs_list, num_expanded_list)
...
def mda_problem_with_astar_experiments():
    ...
    astarMaxAirDist = AStar(MDAMaxAirDistHeuristic)
    resMaxAirDist = astarMaxAirDist.solve_problem(moderate_mda_problem_with_distance_cost)
    print(resMaxAirDist)

    # Ex.20
    # DONE: create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astarSumAirDist = AStar(MDASumAirDistHeuristic)
    resSumAirDist = astarSumAirDist.solve_problem(moderate_mda_problem_with_distance_cost)
    print(resSumAirDist)

    # Ex.23
    # DONE: create an instance of `AStar` with the `MDAMSTAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astarMSTAirDist = AStar(MDAMSTAirDistHeuristic)
    resMSTAirDist = astarMSTAirDist.solve_problem(moderate_mda_problem_with_distance_cost)
    print(resMSTAirDist)
...
def multiple_objectives_mda_problem_experiments():
    ...
    astar_nearest_lab = AStar(MDATestsTravelDistToNearestLabHeuristic)
    ...
    #       Solve the `moderate_mda_problem_with_tests_travel_dist_cost` with it and print the results.
    astar_air_dist = AStar(MDAMSTAirDistHeuristic)
    res = astar_air_dist.solve_problem(moderate_mda_problem_with_distance_cost)
    optimal_distance_cost = res.solution_g_cost
    eps = 0.6
    max_distance_cost = (1 + eps) * optimal_distance_cost
    astar_nearest_lab = AStar(MDATestsTravelDistToNearestLabHeuristic, open_criterion=lambda node: node.cost.distance_cost <= max_distance_cost)
    res = astar_nearest_lab.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
