...
class MDAMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        maxDist = max( self.cached_air_distance_calculator.get_air_distance_between_junctions(j1,j2)
...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        while len(all_certain_junctions_in_remaining_ambulance_path) > 0:
            ...
            remaining_distance_estimation += tmp_list[0][0]
            current_node = tmp_list[0][1]
            all_certain_junctions_in_remaining_ambulance_path.discard(tmp_list[0][1])

        return remaining_distance_estimation
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        """
        DONE [Ex.22]: Implement this method.
